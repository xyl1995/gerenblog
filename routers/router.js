...
router.get("/canvas", user.keepLog, async ctx =>{
  await ctx.render("canvas", {
    title:"拖拽MP3进来播放"
  })
})
//博客日志
router.get("/log", user.keepLog, article.getli)
//博客好文
router.get("/GWriting", user.keepLog, article.getlo)
//博客资源
router.get("/resource", user.keepLog, article.getls)
//关于博主
router.get("/blogger", user.keepLog, async ctx => {
  await ctx.render("blogger", {
    title:"博主大帅比"
  })
})
